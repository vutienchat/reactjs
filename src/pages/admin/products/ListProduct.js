...
import { getList } from "../../../features/products/productSlice";
const ListProduct = () => {
  const [filters, setFilters] = useState({ _limit: 4, _page: 1 });
  ...
  const remove = async (id) => {
    ...
    try {
      await ProductApi.delete(token, id, user._id);
      const newListProduct = listProduct.filter(
        (product) => product._id !== id
      );
      dispatch(getList(newListProduct));
  ...
  useEffect(() => {
    const getListProduct = async () => {
      console.log(process.env.REACT_APP_API_IMG_PRODUCT);
      try {
